...
\begin{itemize}
    ...
    \begin{itemize}[label=$\circ$]
        \item Trường hợp tốt nhất: $O\left(n\log{n}\right)$ vì khi xây dựng heap 
        và thực hiện thao tác heapify vẫn cần $O\left(n \log n\right)$ do tính 
        chất của heap.
        \item Trường hợp xấu nhất: $O\left(n\log{n}\right)$ trong trường hợp tệ 
        nhất (mảng hoàn toàn ngược hoặc có cấu trúc phức tạp) vẫn duy 
        trì $O\left(n \log n\right)$ vì mọi thao tác chính đều dựa trên việc 
        heapify từng phần tử.
        \item Trường hợp trung bình: $O\left(n\log{n}\right)$ trường hợp trung bình 
        vẫn cần xây dựng heap và thực hiện $n-1$ lần thao tác Heapify.
    \end{itemize}
    \item Độ phức tạp không gian: $O\left(1\right)$ vì Heap Sort là thuật toán 
